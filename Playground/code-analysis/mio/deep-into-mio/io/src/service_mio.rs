...
const NUMBER_OF_SOCKET_WORKERS: usize = 16;
...
where Message: Send + Sync + 'static
{
    ...
    pub fn start(
        ...
        let workers = (0..NUMBER_OF_WORKERS)
            .map(|i| {
                Worker::new(
                    i,
                    stealer.clone(),
        ...
        let socket_workers = (0..NUMBER_OF_SOCKET_WORKERS)
            .map(|i| {
    ...
    fn timeout(&mut self, event_loop: &mut EventLoop<Self>, token: Token) {
        ...
        if let Some(handler) = self.handlers.read().get(handler_index) {
            ...
            if let Some(timer) = maybe_timer {
                ...
                super::NET_IO_WORKER_QUEUE.enqueue(1);
    ...
    fn notify(&mut self, event_loop: &mut EventLoop<Self>, msg: Self::Message) {
        match msg {
            ...
            IoMessage::RemoteMessage {
                ...
                //FIXME: looking for a better way to decouple peer and socket
                let worker_id: usize =
                    rng.gen_range(0, NUMBER_OF_SOCKET_WORKERS);
                if let Some(handler) = self.handlers.read().get(handler_id) {
                    ...
                    super::NET_SOCKET_WORKER_QUEUE.enqueue(1);
                }
            }
        }
    }
